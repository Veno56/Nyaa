...
export default new class Sukebei extends AbstractSource {
  name = 'Sukebei (NSFW)'
  description = 'Sukebei is a public, closed tracker. Offers no good API or searches, and its searches are generally accurate.'
  ...
  async single ({ anilistId, media, episode, exclusions, resolution, mode, ignoreQuality }) {
    ...
    const baseUrl = `${this.url}?page=rss&c=1_1&f=0&s=seeders&o=desc&q=(${titles})${ep}${quality ? quality : ''}`;
    ...
    if (absolute) {
      ...
      const baseUrl = `${this.url}?page=rss&c=1_1&f=0&s=seeders&o=desc&q=(${titles})${Utils.epstring(absoluteep)}${quality ? quality : ''}`;
      const url = new URL(excl ? `${baseUrl}-(${excl})` : baseUrl);
